...
* The _committee_ `votes_to_add`,`votes_to_remove`, and `num_committee`, if present, are valid:
	* if `new_committee_account` = `GRAPHENE_PROXY_TO_SELF_ACCOUNT`, and
	* if 0 &le; `new_num_committee` &le; size of new committee slate &le; `chain_params.maximum_committee_count`
...
* The _witness_ `votes_to_add`,`votes_to_remove`, and `num_witness`, if present, are valid if:
	* if `new_witness_account` = `GRAPHENE_PROXY_TO_SELF_ACCOUNT`, and
	* if 0 &le; `new_num_witness` &le; size of new witness slate &le; `chain_params.maximum_witness_count`
