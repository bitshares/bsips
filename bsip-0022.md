...
Vote power tallying retain the existing schedule for vote tallying which is occurs during every maintenance interval.
...
Votes for worker proposals by a proxy shall be tabulated with the full cumulative power of the proxy account plus the full delegated voting power of its delegators [because worker proposals are outside the scope of this proposal](#out-of-scope).
