...
* `account`(account_type): This account pays the additional collateral and
...
5. It creates a `collateral_bid_object` containing the `account` and the partial
...
Obviously, the operation must be authorized by `account`.
...
In order to make this event visible in the bid owners' account histories, all
of these steps are executed using virtual `bid_collateral_operation` and
...
the collateral can be shared among multiple participants.
