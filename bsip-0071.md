...
(a) only takes over the bad debt positions and moves them to a settlement pool without touching the debt positions with CR>1, and 
...
Introduce new parameters: *threshold price*, *settlement price* and *call execution price*. While the *threshold price* is a new BitAsset option and defined by the asset owner, the remaining two shall be calculated as:
