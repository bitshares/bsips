...
Introduce new parameters: *threshold price*, *settlement price* and *call execution price*. While *threshold price* is defeined by the asset owner as bitasset option. The remaining two shall be calculated as:
...
It is important to eliminate concerns about global settlement.  This is currently achieved with the technical options that are available to the price feeders (i.e. the witnesses for BitASsets like bitCNY).  A more advanced solution as proposed here is needed that allows the price feeders to feed the real external market price.
